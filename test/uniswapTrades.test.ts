...
import { SwapRouter, UniswapTrade, UNIVERSAL_ROUTER_ADDRESS } from '../src'
...
import { buildTrade, getUniswapPools, swapOptions, ETHER, DAI, USDC, WETH, getPair } from './utils/uniswapData'
...
describe('Uniswap', () => {
  ...
  let accounts: any
  let user: any
  let tokenATokenBPair: Pair
  let tokenA: Token
  let tokenB: Token
  let FORK_BLOCK: number
  let tokenBContract: Contract
  before(async () => {
    FORK_BLOCK = 17961885;
    const ACCOUNT_ADDRESS = '0x714791780B78E582c855EBe600657c5379b770Ef'
    ...
    user = await ethers.getSigner(ACCOUNT_ADDRESS)
    // Send user some fund if user has no fund
    // await accounts[0].sendTransaction({
    //   to: user.address,
    //   value: parseEther('1'),
    // })

    //@note get pair of uniswapV2
    tokenA = new Token(1, '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2', 18, '', '')
    tokenB = new Token(1, '0x6B175474E89094C44Da98b954EedeAC495271d0F', 18, '', '')
    tokenATokenBPair = await getPair(tokenA, tokenB, FORK_BLOCK)
  })
  describe('v2', () => {
    it('encodes a single exactInput ETH->USDC swap', async () => {
      // USDC contract instance
      tokenBContract = new Contract(tokenB.address, ERC20_ABI, user)
      const userTokenBBalanceBeforeSwap = await tokenBContract.balanceOf(user.address)
      console.log('ETH balance of user before swap', formatEther(await user.getBalance()))
      console.log('userTokenBBalanceBeforeSwap', userTokenBBalanceBeforeSwap.toString())
      const inputEther = utils.parseEther('1').toString()
      const tokenAmounts = tokenATokenBPair.getOutputAmount(CurrencyAmount.fromRawAmount(WETH, inputEther))
      console.log('Expected output', tokenAmounts[0].toExact())
      const trade = new V2Trade(
        new RouteV2([tokenATokenBPair], ETHER, tokenB),
        CurrencyAmount.fromRawAmount(ETHER, inputEther),
        TradeType.EXACT_INPUT
      )
      const opts = swapOptions({}, user.address)
      ...
      const userTokenBBalanceAfterSwap = await tokenBContract.balanceOf(user.address)
      console.log('ETH balance of user after swap', formatEther(await user.getBalance()))
      console.log('userTokenBBalanceAfterSwap', formatUnits(userTokenBBalanceAfterSwap.toString(), 6))
    })
    ...
    it('encodes a single exactInput ETH->USDC swap, with a fee', async () => {
      ...
      const feeOptions: FeeOptions = { fee: new Percent(5, 100), recipient: accounts[0].address }
      const opts = swapOptions({fee: feeOptions}, user.address)
      // const methodParameters = SwapRouter.swapERC20CallParameters(buildTrade([trade]), opts)
      const methodParameters = SwapRouter.swapCallParameters(new UniswapTrade(buildTrade([trade]), opts))
      const tx = await user.sendTransaction({
        from: user.address,
      ...
      const userTokenBBalanceAfterSwap = await tokenBContract.balanceOf(user.address)
      console.log('ETH balance of user after swap', formatEther(await user.getBalance()))
      console.log('userTokenBBalanceAfterSwap', formatUnits(userTokenBBalanceAfterSwap.toString(), tokenB.decimals))
      console.log('fee recipient token balance', formatUnits((await tokenBContract.balanceOf(accounts[0].address)).toString(), tokenB.decimals))
  ...
  //   it.skip('encodes a split exactInput with 2 routes v3ETH->v3USDC & v2ETH->v2USDC swap', async () => {
  ...
  //   it.skip('encodes a split exactInput with 3 routes v3ETH->v3USDC & v2ETH->v2USDC swap', async () => {
  //     const inputEther = expandTo18Decimals(1)
  //     const v2Trade = new V2Trade(
  //       new RouteV2([WETH_USDC_V2], ETHER, USDC),
  //       CurrencyAmount.fromRawAmount(ETHER, inputEther),
